...
function interpret_center_of_composition_statement(center::Expr)::Tuple{Symbol, Expr} # sf, foot defintion
  @assert length(center.args) == 3 && center.args[1] == :(=>) && typeof(center.args[2]) == Expr 
    "Invalid argument: expected A ^ B => C, A ^ () => C or A ^ B => (), got $center"
  # third argument can be symbol or (), the latter of which is an Expr
  center_caret_statement = center.args[2]
  @assert (length(center_caret_statement.args) == 3 
    && center_caret_statement.args[1] == :^
    && typeof(center_caret_statement.args[2]) == Symbol) 
    "Invalid center argument: expected A ^ B or A ^ (), got $center"
...
function interpret_composition_notation(mapping_pair::Expr)::Tuple{Vector{Symbol}, StockAndFlow0}
  ...
  @assert center_index <= length(expr_args) 
    "A tuple is an invalid expression for composition syntax.  \
    Expected argument of form sf1, sf2, ... ^ stock1 => sum1, stock2 => sum2, ..."
...
"""
sirv = sfcompose(sir, svi, quote (sr, sv) sr, sv ^ S => N, I => N end)

Cannot use () => () as a foot, the length of the first tuple must be the same as
the number of stock flows given as argument, and every foot can only be used
once.
"""
function sfcompose(sfs::Vector{K}, block::Expr) where {K <: AbstractStockAndFlowF}#(sf1, sf2, ..., block)
  ...
  # I'd prefer this to be a vector, but oapply didn't like that I'd also prefer
  # that I don't include the empty foot, but Open doesn't want to accept
  # stockflows with no feet. open_stockflows::AbstractDict = Dict(sf_key =>
  # Open(sf_val, foot_dict[sf_val]...,) for (sf_key, sf_val) ∈ sf_map)
