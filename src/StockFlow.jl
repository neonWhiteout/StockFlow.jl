...
sindex(p::AbstractStockAndFlow0,s) = findfirst(x -> Symbol(x) == s, snames(p)) # return first index of stock with equal name
findex(p::AbstractStockAndFlowStructure,f) =  findfirst(x -> Symbol(x) == s, fnames(p)) # return first index of flow with equal name
svindex(p::AbstractStockAndFlow0,sv) = findfirst(x -> Symbol(x) == s, svnames(p)) # return first index of sum variable with equal name
vindex(p::AbstractStockAndFlowStructure,v) =  findfirst(x -> Symbol(x) == s, vnames(p)) # return first index of dynamic variable with equal name
pindex(sf::AbstractStockAndFlowStructureF,p) =  findfirst(x -> Symbol(x) == s, pnames(p)) # return first index of parameters with equal name
